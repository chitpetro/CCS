...
namespace GUI
{
    public partial class f_hopdong_cdt : Form
    {
        private KetNoiDBDataContext db = new KetNoiDBDataContext();
        private readonly t_hopdong_cdt hd = new t_hopdong_cdt();

        public f_hopdong_cdt()
        {
            InitializeComponent();
            ...
            var quyen1 =
                db.PhanQuyen2s.FirstOrDefault(
                    p => p.TaiKhoan == Biencucbo.phongban && p.ChucNang == "HopDong_CDT_TheoDoiThanhToan");

            if (quyen1 == null)
            {
                quyen1 = new PhanQuyen2();
                quyen1.TaiKhoan = Biencucbo.phongban;
                quyen1.ChucNang = "HopDong_CDT_TheoDoiThanhToan";
        ...
        public void load()
        {
            ...
            try
            {
                //var lst = (from a in db.hopdong_tps where a.iddv == Biencucbo.donvi && a.idct == Biencucbo.mact select a.so).Max();
                var lst1 =
                    (from b in db.hopdong_cdts where b.iddv == Biencucbo.donvi select b).Single(
                        t => t.id == Biencucbo.ma);
                if (lst1 == null) return;

                gcchitiet.DataSource = lst1.hopdongct_cdts;
        ...
        private void btnnew_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            gcchitiet.DataSource = new KetNoiDBDataContext().View_hdcdts; //.r_hopdongs;
        ...
        public void luu()
        {
            ...
            else
            {
                if (Biencucbo.hdhdtp == 0)
                {
                    ...
                    try
                    {
                        var check = "HDCDT" + Biencucbo.donvi.Trim();
                        ...
                        hd.moihd(txtid.Text.Trim(), txtNgaylap.DateTime, txtngaybd.DateTime, txtngaykt.DateTime,
                            ...
                            txtsohd.Text, txttiente.Text, double.Parse(txttygia.Text), Biencucbo.mact, txtnoidung.Text,
                            slklhd.Text, slulink.Text);

                        for (var i = 0; i <= gridView1.RowCount - 1; i++)
                        {
                            gridView1.SetRowCellValue(i, "id", txtid.Text + i);
                            gridView1.SetRowCellValue(i, "idhd_cdt", txtid.Text);

                            hd.moihdct(gridView1.GetRowCellValue(i, "id").ToString(),
                                gridView1.GetRowCellValue(i, "idhd_cdt").ToString(),
                        ...
                        // History 
                        hs.add(txtid.Text, "Thêm mới Hợp đồng Chủ đầu tư");
                    }
                    catch (Exception ex)
                    {
                ...
                else //Sua HD
                {
                    try
                    {
                        ...
                        hs.add(txtid.Text, "Sửa Hợp đồng Chủ đầu tư");
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show(ex.ToString());
        ...
        private void btnsua_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            try
            {
                var lst = (from pn in db.hopdong_cdts select pn).Single(x => x.id == txtid.Text);
                if (lst == null) return;
                gcchitiet.DataSource = lst.hopdongct_cdts;
                //enabled
                txtghichu.ReadOnly = false;
                txtnoidung.ReadOnly = false;
                slklhd.ReadOnly = false;
        ...
        private void btnload_ItemClick(object sender, ItemClickEventArgs e)
        {
            if (Biencucbo.hdhdtp == 1)
            {
                ...
                var lst = (from pn in db.hopdong_cdts select pn).FirstOrDefault(x => x.id == txtid.Text);
            ...
            if (Biencucbo.hdhdtp != 2)
            {
                ...
                else if (e.KeyCode == Keys.Delete)
                {
                    if (Biencucbo.hdhdtp == 1)
                    {
                        ...
                        {
                            var ct =
                                (from c in db.hopdongct_cdts select c).Single(
        ...
        private void gridView1_InitNewRow(object sender, InitNewRowEventArgs e)
        {
            if (Biencucbo.hdhdtp == 1)
            {
                var ct = gridView1.GetFocusedRow() as hopdongct_cdt;
                if (ct == null) return;
        ...
        private void f_hd_Load(object sender, EventArgs e)
        {
            ...
            Text = LanguageHelper.TranslateMsgString("." + Name + "_title", "Hợp Đồng Chủ Đầu Tư");
            ...
            if (Biencucbo.getID == 1)
            {
                ...
                {
                    Biencucbo.hdhdtp = 2;
                    var lst = (from hd in db.hopdong_cdts select new {a = hd}).Single(x => x.a.id == Biencucbo.ma);
                    ...
                    gcchitiet.DataSource = lst.a.hopdongct_cdts;
