...
namespace GUI
{
    public partial class f_vanbandinb : Form
    {
        ...
        private readonly t_vanbanden vbden = new t_vanbanden();
        private readonly t_vanbandi vbdi = new t_vanbandi();

        public f_vanbandinb()
        {
            InitializeComponent();
            var lst = from a in db.doituongs where a.nhom == "Nội Bộ Văn Phòng" select a;
            txtiddt.Properties.DataSource = _tTodatatable.addlst(lst.ToList());
        ...
        public void luu()
        {
            ...
            else
            {
                        ...
                        try
                        {
                            ...
                            if (Biencucbo.linkvb == 1)
                            {
                                ...
                                vbdi.moivbdi(txtid.Text.Trim(), txtdv.Text, txtIdLoai.Text, txtNgaygui.DateTime,
                                    ...
                                    Convert.ToInt32(txt1.Text), Biencucbo.ma, true, txtghichu.Text, txtlydo.Text, txttenct.Text, txttendoitac.Text);
                                ls.suadithem(Biencucbo.ma, txtiddt.Text, txtNgaygui.DateTime, txtidnv.Text, txtid.Text,
                                    txtlydo.Text);

                                vbden.moivbd(txtid.Text.Trim(), txtiddt.Text, txtIdLoai.Text, txtNgaygui.DateTime, "",
                                    txtsovb.Text, txtdv.Text, txtnoidung.Text, txttrichyeu.Text,
                                    Convert.ToInt32(txt1.Text), true, false, txtghichu.Text, txtlydo.Text, txttenct.Text, txttendoitac.Text);
                                var lst = from a in db.lshosos where a.id.Contains(Biencucbo.ma) select a.buoc;
                                if (lst.Count() > 0)
                                {
                                    buoc = int.Parse(lst.Max().ToString()) + 1;
                                    var lst2 =
                                        (from a in db.lshosos where a.id.Contains(Biencucbo.ma) select a).Single(
                                            t => t.buoc == buoc - 1);
                                    var lst3 =
                                        (from a in db.lshosos where a.hsgoc == lst2.hsgoc select a).Single(
                                            t => t.buoc == 1);
                                    ls.moi(lst2.id + "." + txtid.Text, txtid.Text, buoc, Biencucbo.donvi,
                                        txtNgaygui.DateTime, "", lst3.id, lst2.iddv + "." + txtiddt.Text, txtiddt.Text,
                                        txtghichu.Text);
                                }
                                else
                                {
                                    ls.moi(txtid.Text, txtid.Text, 1, txtiddt.Text, txtNgaygui.DateTime, txtidnv.Text,
                                        txtid.Text, Biencucbo.donvi, txtiddt.Text, txtghichu.Text);
                                }
                            }
                            else
                            {
                                vbdi.moivbdi(txtid.Text.Trim(), txtdv.Text, txtIdLoai.Text, txtNgaygui.DateTime,
                                    txtidnv.Text, txtsovb.Text, txtiddt.Text, txtnoidung.Text, txttrichyeu.Text,
                                    Convert.ToInt32(txt1.Text), "", true, txtghichu.Text, txtlydo.Text, txttenct.Text, txttendoitac.Text);
                                ls.moidi(txtid.Text, txtid.Text, 1, txtiddt.Text, txtNgaygui.DateTime, txtidnv.Text,
                                    txtid.Text, txtid.Text, Biencucbo.donvi, Biencucbo.donvi, txtlydo.Text);
                                vbden.moivbd(txtid.Text.Trim(), txtiddt.Text, txtIdLoai.Text, txtNgaygui.DateTime, "",
                                    txtsovb.Text, txtdv.Text, txtnoidung.Text, txttrichyeu.Text,
                                    Convert.ToInt32(txt1.Text), true, false, txtghichu.Text, txtlydo.Text, txttenct.Text, txttendoitac.Text);
                                ls.moi(txtid.Text + "." + "2", txtid.Text, 2, Biencucbo.donvi, txtNgaygui.DateTime, "",
                                    txtid.Text, Biencucbo.donvi + "." + txtiddt.Text, txtiddt.Text, "");
                            }
                            for (var i = 0; i < gridView1.DataRowCount; i++)
                            {
                                ...
                                vbden.luuct(txtid.Text + i, tenfile, (Binary)gridView1.GetRowCellValue(i, "formData"),
                                    gridView1.GetRowCellValue(i, "formSize").ToString(), diengiai, txtid.Text);
                            }
                            try
                            {
                ...
                else if (Biencucbo.vbdi == 1)  //Sua VB Di
                {
                    try
                    {
                        ...
                        vbden.suavbd(txtid.Text.Trim(), txtiddt.Text, txtIdLoai.Text, txtNgaygui.DateTime, "",
                            txtsovb.Text, txtdv.Text, txtnoidung.Text, txttrichyeu.Text,
                            Convert.ToInt32(txt1.Text), txtghichu.Text, txtlydo.Text, txttenct.Text, txttendoitac.Text);
                        ...
                        try
                        {
                            //if (Biencucbo.linkvb == 1)
                            //{
                            //    var lst23 = from a in db.lshosos where a.id.Contains(Biencucbo.ma) select a.buoc;
                            //    if (lst23.Count() > 0)
                            //    {
                            //        int buoc = 0;
                            //        buoc = int.Parse(lst23.Max().ToString()) + 1;
                            //        var lst2 =
                            //            (from a in db.lshosos where a.id.Contains(Biencucbo.ma) select a).Single(
                            //                t => t.buoc == buoc - 1);
                            var lst3 = (from a in db.lshosos select a).Single(t => t.idvbdi == txtid.Text);
                            var lsttest = (from a in db.lshosos where a.id.Contains(lst3.id) select a);
                            var lst2 = (from a in db.lshosos where a.id.Contains(lst3.id) select a).Single(t => t.buoc == lst3.buoc + 1);

                            ls.suaden2(lst2.id, txtiddt.Text, txtNgaygui.DateTime,lst3.iddv + "." + txtiddt.Text);
                            //    }
                            //}
                        }
                        catch (Exception ex)
                        ...
                        hs.add(txtid.Text, "Sửa Văn Bản Đi");
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show(ex.ToString());
        ...
        private void f_vanbandinb_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Control)
            {
                if (e.KeyCode == Keys.O)
        ...
        //    {
        //    if (lst3.Count() != 0)
        //    var lst3 = (from dt in db.vanbandens where dt.duyet == true select dt).ToList();
        //    db = new KetNoiDBDataContext();
        //{
