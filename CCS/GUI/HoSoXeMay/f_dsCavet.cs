...
{
    public partial class f_dsCavet : frmdsmo
    {
        ...
        protected override void search()
        {
            try
            {
                gd.DataSource = (from a in dbData.cavets where a.idpt == Biencucbo.idpt && a.ngaydk >= DateTime.Parse(tungay.EditValue.ToString()) && a.ngaydk <= DateTime.Parse(denngay.EditValue.ToString()) select a);
                gv.ExpandAllGroups();
                gv.BestFitColumns();
            }
            catch { }
        ...
        protected override void searchall()
        {
            try
            {
                gd.DataSource = (from a in dbData.cavets where a.idpt == Biencucbo.idpt select a);
                gv.ExpandAllGroups();
                gv.BestFitColumns();
            }
            catch { }
