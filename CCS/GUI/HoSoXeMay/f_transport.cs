...
{
    public partial class f_transport : frmp
    {

        c_transport bh = new c_transport();
        ...
        private void xoatxt()
        {
            ...
            dataLayoutControl1.DataSource = (from a in db.transports where a.key == _key select a);
            idTextEdit.Text = "";
            ngaydkDateEdit.Text = "";
        ...
        private void themtxt()
        {
            _keytemp = _key;
            _key = MD5.laykey();
            gd.DataSource = (from a in db.transport_files where a.keyts == _key select a);
            xoatxt();
        ...
        private void loadinfo(string key)
        {
            try
            {
                var lst = (from a in new KetNoiDBDataContext().transports select a).Single(t => t.key == key);
                //txtid.Text = lst.id;
                ...
                gd.DataSource = lst.transport_files;
                _key = lst.key;
        ...
        private bool kiemtra()
        {
            ...
            var lst = (from a in new KetNoiDBDataContext().transports where a.idpt == _mapt select a);
        ...
        private void btnluufile_Click(object sender, EventArgs e)
        {
            if (_hdong != 2)
            {
                ...
                var ct = gv.GetFocusedRow() as transport_file;
                ct.key = MD5.laykey();
                ct.keyts = _key;
        ...
        private void btntaifile_Click(object sender, EventArgs e)
        {
            if (_hdong == 2)
            {
                try
                {
                    var row = gv.GetFocusedRow() as transport_file;
                    if (row == null) return;

                    var a1 = row.key;
                    var lst = (from a in db.transport_files select a).Single(x => x.key == a1);
        ...
        private void btnxoafile_Click(object sender, EventArgs e)
        {
...

            {
                try
                {
                    ...
                    {
                        var ct =
                        (from c in db.transport_files select c).Single(
                            x => x.key == gv.GetFocusedRowCellValue("key").ToString());
                        db.transport_files.DeleteOnSubmit(ct);
        ...
        protected override void load()
        {
            ...
            try
            {
                var so = (from a in db.transports where a.idpt == _mapt select a.so).Max();
                if (so == null)
                    return;
                var lst = (from a in db.transports where a.idpt == _mapt select a).Single(t => t.so == so);
                loadinfo(lst.key);
            }
            catch (Exception ex)
            {
        ...
        protected override void mo()
        {
            ...
            f_dsTransport frm = new f_dsTransport();
        ...
        private bool LuuPhieu()
        {
            ...
            try
            {
                var c1 = db.transport_files.Context.GetChangeSet();
                db.transport_files.Context.SubmitChanges();
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                if (kiemtra())
                {
                    if (_hdong == 0)
                    {
                        ...
                        LuuPhieu();
                        hs.add(idTextEdit.Text, "Thêm Transport");
                    ...
                    if (_hdong == 1)
                    {
                        ...
                        LuuPhieu();
                        hs.add(idTextEdit.Text, "Sữa Transport");
                        XtraMessageBox.Show("Done");
                        dongedit();
                        _hdong = 2;
                        return true;
        ...
        protected override bool xoa()
        {
            ...
            try
            {
                ...
                bh.xoa(_key);
                hs.add(idTextEdit.Text, "Xóa Transport");
                _hdong = 2;
                XtraMessageBox.Show("Done");
                dongedit();
                xoatxt();
        ...
        protected override void top()
        {
            try
            {
                var lst = (from a in db.transports where a.idpt == _mapt select a.so).Min();
                if (lst == null)
                    return;
                var lst1 = (from a in db.transports where a.idpt == _mapt select a).Single(t => t.so == lst);
                loadinfo(lst1.key);
            }
            catch (Exception ex)
            {
        ...
        protected override void prev()
        {
            try
            {
                var lst = (from a in db.transports where a.idpt == _mapt && a.so < _so select a.so).Max();
                if (lst == null)
                {
                    XtraMessageBox.Show("Đây là phiếu đầu tiên");
                    return;
                }
                var lst1 = (from a in db.transports where a.idpt == _mapt && a.so == lst select a).Single();
                loadinfo(lst1.key);
            }
            catch (Exception ex)
            {
        ...
        protected override void next()
        {
            try
            {
                var lst = (from a in db.transports where a.idpt == _mapt && a.so > _so select a.so).Min();
                if (lst == null)
                {
