...
{
    public partial class f_giaydiduong : frmp
    {

        c_giaydiduong bh = new c_giaydiduong();
        ...
        private void xoatxt()
        {
            ...
            dataLayoutControl1.DataSource = (from a in db.giaydiduongs where a.key == _key select a);
            idTextEdit.Text = "";
            ngaydkDateEdit.Text = "";
        ...
        private void themtxt()
        {
            _keytemp = _key;
            _key = MD5.laykey();
            gd.DataSource = (from a in db.giaydiduong_files where a.keygdd == _key select a);
            xoatxt();
        ...
        private void loadinfo(string key)
        {
            try
            {
                var lst = (from a in new KetNoiDBDataContext().giaydiduongs select a).Single(t => t.key == key);
                //txtid.Text = lst.id;
                ...
                gd.DataSource = lst.giaydiduong_files;
                _key = lst.key;
        ...
        private bool kiemtra()
        {
            ...
            var lst = (from a in new KetNoiDBDataContext().giaydiduongs where a.idpt == _mapt select a);
        ...
        private void btnluufile_Click(object sender, EventArgs e)
        {
            if (_hdong != 2)
            {
                ...
                var ct = gv.GetFocusedRow() as giaydiduong_file;
                ct.key = MD5.laykey();
                ct.keygdd = _key;
        ...
        private void btntaifile_Click(object sender, EventArgs e)
        {
            if (_hdong == 2)
            {
                try
                {
                    var row = gv.GetFocusedRow() as giaydiduong_file;
                    if (row == null) return;

                    var a1 = row.key;
                    var lst = (from a in db.giaydiduong_files select a).Single(x => x.key == a1);
        ...
        private void btnxoafile_Click(object sender, EventArgs e)
        {
            ...
            if (_hdong != 2)
            {
                try
                {
                    ...
                    {
                        var ct =
                        (from c in db.giaydiduong_files select c).Single(
                            x => x.key == gv.GetFocusedRowCellValue("key").ToString());
                        db.giaydiduong_files.DeleteOnSubmit(ct);
        ...
        protected override void load()
        {
            ...
            try
            {
                var so = (from a in db.giaydiduongs where a.idpt == _mapt select a.so).Max();
                if (so == null)
                    return;
                var lst = (from a in db.giaydiduongs where a.idpt == _mapt select a).Single(t => t.so == so);
                loadinfo(lst.key);
            }
            catch (Exception ex)
            {
        ...
        protected override void mo()
        {
            ...
            f_dsgiaydiduong frm = new f_dsgiaydiduong();
        ...
        private bool LuuPhieu()
        {
            ...
            try
            {
                var c1 = db.giaydiduong_files.Context.GetChangeSet();
                db.giaydiduong_files.Context.SubmitChanges();
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                if (kiemtra())
                {
                    ...
                    if (_hdong == 1)
                    {
                        ...
                        LuuPhieu();
                        hs.add(idTextEdit.Text, "Sữa Giấy Đi Đường");
                        XtraMessageBox.Show("Done");
                        dongedit();
                        _hdong = 2;
                        return true;
        ...
        protected override bool xoa()
        {
            ...
            try
            {
                ...
                bh.xoa(_key);
                hs.add(idTextEdit.Text, "Xóa Giấy Đi Đường");
                _hdong = 2;
                XtraMessageBox.Show("Done");
                dongedit();
                xoatxt();
        ...
        protected override void top()
        {
            try
            {
                var lst = (from a in db.giaydiduongs where a.idpt == _mapt select a.so).Min();
                if (lst == null)
                    return;
                var lst1 = (from a in db.giaydiduongs where a.idpt == _mapt select a).Single(t => t.so == lst);
                loadinfo(lst1.key);
            }
            catch (Exception ex)
            {
        ...
        protected override void prev()
        {
            try
            {
                var lst = (from a in db.giaydiduongs where a.idpt == _mapt && a.so < _so select a.so).Max();
                if (lst == null)
                {
                    XtraMessageBox.Show("Đây là phiếu đầu tiên");
                    return;
                }
                var lst1 = (from a in db.giaydiduongs where a.idpt == _mapt && a.so == lst select a).Single();
                loadinfo(lst1.key);
            }
            catch (Exception ex)
            {
        ...
        protected override void next()
        {
            try
            {
                var lst = (from a in db.giaydiduongs where a.idpt == _mapt && a.so > _so select a.so).Min();
                if (lst == null)
                {
